...
public:
    ...
    void CfgFileReader ()
    {
        ...
        std::string pack_path = ros::package::getPath("model_loader");
        std::cout << "\tThe path for package [model_loader] is " << pack_path << std::endl;
        cfgFileName = pack_path + "/config/aircraft_frame_setup.cfg";
        std::cout << "\tThe path of the configuration file is: [" << cfgFileName << "]" << std::endl;
  	    ...
  	    if (std::getline(input, line)) {
              ...
              for(int idx = 0; idx < objectNum; idx++) {
                  ...
                  roll = rollD * M_PI / 180.0; // rollD degrees around new X
                  pitch = pitchD * M_PI / 180.0; // pitchD degrees around new Y
...
{
    ros::init(argc, argv, "AR_Aircraft_frame");
    ros::NodeHandle node;
    ros::AsyncSpinner spinner(1);
    spinner.start();

    ROS_INFO("Starting Add and Remove Aircraft frame");
    AddAndRemoveObject testClass;
    testClass.CfgFileReader();
