...
public:
  ...
  void segment_list_cb ( const object_localizer_msg::Segment_list::ConstPtr& segment_list )
  {
    if ( segment_list->BBox_list_float.size() > 0 )
    {
      ...
      // step 2, let user choose to scan which segments
      std::cout << "Choose a segment to scan >> please enter the id of the segmen <<" << std::endl;
      int segment_id;
      std::cin >> segment_id;

      // step 3, generate scan plan for the chosen segment
      pcl::PCLPointCloud2 pcl_pc2;
      pcl_conversions::toPCL( segment_list->Segment_list [ segment_id ], pcl_pc2 );
  ...
  ScanPlanner () : segment_list_topic_ ( "/box_segmenter/segment_list" )
  {
    segment_list_sub_ = nh_.subscribe ( segment_list_topic_, 6, &ScanPlanner::segment_list_cb, this );
    std::string segment_in_name = nh_.resolveName ( segment_list_topic_ );
...
private:
  ros::NodeHandle nh_;
  std::string segment_list_topic_;
